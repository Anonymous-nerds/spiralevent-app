...
const RegisterForm = () => {
  ...
  const [data, setData] = useState({ name: "", username: "", email: "", password: "", conPassword: "", });
    ...
    <div className="RegisterForm">
      <form onSubmit={handleSubmit}
        ...
        <div className="space-y-4">
          ...
          <input type="password" placeholder="Confirm Password" className={`inputForm ${borderColors.conPassword}`}
            value={data.conPassword} onChange={(e) => setData({ ...data, conPassword: e.target.value })}
          />
          {errors.conPassword && (<span className="text-red-500 text-sm">{errors.conPassword}</span>)}
        </div>

        <button type="submit" className="submitBtn">Register</button>
        <p className="text-sm text-gray-600 flex gap-2 justify-center mt-4">
          Already have an account?{" "}
          <Link to="/login" className="text-pink-900 underline">Log in</Link>
        </p>
      </form>
    </div>
  );
